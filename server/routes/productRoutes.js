...
// Validates and parses purchaseRate. `required` = true for create, false for partial update.
function parsePurchaseRate(purchaseRate, required) {
    if (purchaseRate === undefined) {
        if (required) {
            return { error: "Purchase rate is required" };
        }
        return { value: undefined }; // not being updated
    }

    const parsed = Number(purchaseRate);
    if (purchaseRate === "" || purchaseRate === null || isNaN(parsed) || parsed <= 0) {
        return { error: "Purchase rate must be a number greater than 0" };
    }
    return { value: parsed };
}
...
router.post("/", async (req, res) => {
    try {
        const { companyId, productCode, name, purchaseRate, ...productData } = req.body;
        ...
        const purchaseRateResult = parsePurchaseRate(purchaseRate, true);
        if (purchaseRateResult.error) {
            return res.status(400).json({ error: purchaseRateResult.error });
        }
        productData.purchaseRate = purchaseRateResult.value;
...
router.put("/:id", async (req, res) => {
    try {
        const { companyId } = req.query;
        const { productCode, name, purchaseRate, ...updateData } = req.body;
        ...
        updateFields.purchaseRate = purchaseRateResult.value;
