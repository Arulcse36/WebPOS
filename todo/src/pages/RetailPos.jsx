...
// Generates a unique id for each cart line so removal/quantity updates
// never depend on comparing product ids or barcode ObjectIds directly.
const makeCartLineId = () => `line_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;

// Normalizes a barcode id (which may be a Mongo ObjectId, a populated
// object, a string, or null/undefined) into a plain string or null so
// group-level comparisons are always safe.
const normalizeBarcodeId = (barcodeDetails) => {
    if (!barcodeDetails || !barcodeDetails._id) return null;
...
const ProductSearchInput = forwardRef(({ 
    ...
    const getDisplayPrice = (product, barcode) => {
        if (!product) return 0;
        if (barcode) {
            if (rateType === "wholesale") {
                return barcode.wholesaleRate || product.wholesaleRate || product.retailRate || 0;
            }
            return barcode.retailRate || product.retailRate || 0;
        }
        if (rateType === "wholesale") {
    ...
    // Updated search logic with priority: Product Code > Barcode > Name
    useEffect(() => {
        if (searchTerm.trim().length > 0 && !selectedProduct) {
            const term = searchTerm.trim().toLowerCase();

            // Create sets to track what's been added
            const seenProductIds = new Set();
            let filteredProducts = [];

            // STEP 1: Find exact product code matches (HIGHEST PRIORITY)
            const exactCodeMatches = products.filter(p =>
                p && p.isActive === true &&
                p.productCode && 
                p.productCode.toString().toLowerCase() === term
            );

            // Add exact code matches first
            exactCodeMatches.forEach(p => {
                if (!seenProductIds.has(p._id)) {
                    filteredProducts.push({
                        ...p,
                        _matchType: 'exact-code',
                        _priority: 1
                    });
            ...
            // STEP 2: Find partial product code matches (if no exact matches)
            if (exactCodeMatches.length === 0) {
                const partialCodeMatches = products.filter(p =>
                    p && p.isActive === true &&
                    p.productCode && 
                    p.productCode.toString().toLowerCase().includes(term)
                );

                partialCodeMatches.forEach(p => {
                    if (!seenProductIds.has(p._id)) {
                        filteredProducts.push({
                            ...p,
                            _matchType: 'partial-code',
                            _priority: 2
                        });
                        seenProductIds.add(p._id);
                    }
            ...
            // STEP 3: Find barcode matches (SECOND PRIORITY)
            // Only if no product code matches were found
            if (exactCodeMatches.length === 0) {
                const matchingBarcodes = barcodes.filter(b => 
                    b && b.isActive !== false &&
                    b.barcode && b.barcode.toString().toLowerCase().includes(term)
                );

                // Exact barcode matches
                const exactBarcodeMatches = matchingBarcodes.filter(b => 
                    b.barcode.toString().toLowerCase() === term
                );

                // Add exact barcode matches
                exactBarcodeMatches.forEach(b => {
                    const productId = b.productId?._id || b.productId;
                ...
                // If no exact barcode matches, add partial barcode matches
                if (exactBarcodeMatches.length === 0) {
                    matchingBarcodes.forEach(b => {
                        const productId = b.productId?._id || b.productId;
                        if (!seenProductIds.has(productId)) {
                            const product = products.find(p => p._id === productId);
                            if (product && product.isActive === true) {
            ...
            // STEP 4: Find name matches (THIRD PRIORITY)
            // Only if no product code matches were found
            if (exactCodeMatches.length === 0) {
                ...
                // Check if this product was already added via barcode
            ...
            filteredProducts.sort((a, b) => (a._priority || 99) - (b._priority || 99));
    ...
    }, [searchTerm, products, barcodes, selectedProduct]);
    ...
    useEffect(() => {
        if (selectedProduct) {
            const defaultPrice = getDisplayPrice(selectedProduct, selectedBarcode);
            setPrice(defaultPrice);
        }
    }, [rateType, selectedProduct, selectedBarcode]);
    ...
    }, [selectedProduct, rateType, selectedBarcode]);
    ...
    const handleSelectProduct = (product) => {
        ...
        setSelectedBarcode(product._matchedBarcode || null);
        setSearchTerm(product.name);
        setShowResults(false);
        setSearchResults([]);
        setQuantity(1);
        const defaultPrice = getDisplayPrice(product, product._matchedBarcode || null);
        setPrice(defaultPrice);
        setSelectedIndex(-1);
    };
    ...
    const handleAdd = () => {
        ...
        const productWithPrice = {
            ...
            retailRate: price,
            isPriceEdited: true,
            barcodeDetails: selectedBarcode || null
        };
                ...
                <div className="flex items-end gap-3">
                    ...
                    <div className="relative flex-[2] min-w-[250px]">
                        <label className="block text-xs font-medium text-gray-600 mb-1.5">
                            🔍 Search Product
                            <span className="ml-2 text-indigo-600 text-[10px] font-normal">(code &gt; barcode &gt; name)</span>
                        </label>
                        <div className="relative">
                            ...
                            <input
                                ref={inputRef}
                                type="text"
                                placeholder={selectedProduct ? selectedProduct.name : "Search by code, barcode, or name..."}
                                value={searchTerm}
...
const CartGrid = ({ items, onUpdateQuantity, onRemoveItem, onEditPrice }) => {
    ...
    // Group items by product and barcode (for display only — every
    // underlying cart line still keeps its own unique `id`).
    const groupedItems = useMemo(() => {
        ...
        items.forEach((item, index) => {
            // Create a unique key: productId + barcodeId (or 'no-barcode')
            const barcodeId = normalizeBarcodeId(item.barcodeDetails) || 'no-barcode';
            const key = `${item.product}_${barcodeId}`;
            
            if (groups.has(key)) {
                const existing = groups.get(key);
                // Merge quantities
                const newQty = roundToTwoDecimals((existing.qty || 0) + (item.qty || 0));
                const newTotal = roundToTwoDecimals((existing.price || 0) * newQty);
                groups.set(key, {
                    ...existing,
                    qty: newQty,
                    total: newTotal,
                    // Keep the barcode details from the first item
                    barcodeDetails: existing.barcodeDetails || item.barcodeDetails,
                    // Keep track of all items in this group
                    _items: [...(existing._items || []), item],
                    _itemIds: [...(existing._itemIds || []), item.id],
                    _originalIndexes: [...(existing._originalIndexes || []), index]
                });
            } else {
                groups.set(key, {
                    ...item,
    ...
    // Update grouped item quantity — adjusts a single underlying cart
    // line (by its unique id) by the requested delta.
    const handleGroupUpdate = useCallback((groupKey, newQty) => {
        const roundedQty = Math.round(newQty * 1000) / 1000;
        
        // Find the group
        const group = groupedItems.find(g => {
            const barcodeId = normalizeBarcodeId(g.barcodeDetails) || 'no-barcode';
            return `${g.product}_${barcodeId}` === groupKey;
        });
        
        if (!group || !group._items || group._items.length === 0) return;
        
        if (roundedQty <= 0) {
            // Remove all items in this group — by unique id, so sibling
            // groups (e.g. the non-barcoded line of the same product)
            // are never touched.
        ...
        const currentTotalQty = group._items.reduce((sum, item) => sum + (item.qty || 0), 0);
        const diff = roundedQty - currentTotalQty;
        
        if (diff !== 0) {
            // Update the first item by the difference — by unique id
            const firstItem = group._items[0];
            const newItemQty = Math.max(0, (firstItem.qty || 0) + diff);
            onUpdateQuantity(firstItem.id, newItemQty);
        }
    }, [groupedItems, onUpdateQuantity, onRemoveItem]);

    // Remove entire group — removes only the exact cart lines belonging
    // to this group, by their unique ids.
    const handleRemoveGroup = useCallback((groupKey) => {
        const group = groupedItems.find(g => {
            const barcodeId = normalizeBarcodeId(g.barcodeDetails) || 'no-barcode';
            return `${g.product}_${barcodeId}` === groupKey;
        });
        if (group && group._items) {
            // Remove each item individually, by its unique id
            group._items.forEach(item => {
                onRemoveItem(item.id);
            });
        }
    }, [groupedItems, onRemoveItem]);

    const startEditQty = (groupKey, currentQty) => {
        setEditingQtyId(groupKey);
        setEditingQtyValue(currentQty.toString());
        setTimeout(() => {
            if (qtyInputRef.current) {
                qtyInputRef.current.focus();
                qtyInputRef.current.select();
    ...
    const saveQtyEdit = (groupKey) => {
        const newQty = parseFloat(editingQtyValue);
        if (!isNaN(newQty) && newQty > 0) {
            handleGroupUpdate(groupKey, newQty);
        }
        setEditingQtyId(null);
        setEditingQtyValue("");
    };

    const handleQtyKeyDown = (e, groupKey) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            saveQtyEdit(groupKey);
                ...
                <table className="w-full">
                    ...
                    <tbody>
                        {groupedItems.map((item, index) => {
                            const groupKey = `${item.product}_${normalizeBarcodeId(item.barcodeDetails) || 'no-barcode'}`;
                            const isEditing = editingQtyId === groupKey;
                            ...
                            const hasBarcode = item.barcodeDetails && item.barcodeDetails.barcode;
                            
                            return (
                                <tr key={groupKey} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-3 py-2">
                                        <div className="font-semibold text-gray-900 text-sm">
                                            ...
                                            {item._items && item._items.length > 1 && (
                                                <span className="ml-1 text-[10px] text-gray-400">
                                                    ({item._items.length} items)
                                                </span>
                                            )}
                                        </div>
                                        {hasBarcode && (
                                            <div className="text-[10px] text-indigo-500 font-mono flex items-center gap-1 mt-0.5">
                                                <span>📱</span>
                                                <span>{safeString(item.barcodeDetails.barcode)}</span>
                                                {item.barcodeDetails.expiryDate && (
                                                    <span className="text-gray-400 ml-1">
                                                        (Exp: {new Date(item.barcodeDetails.expiryDate).toLocaleDateString()})
                                                    </span>
                                                )}
                                            </div>
                                        )}
                                        <div className="text-[10px] text-gray-400 mt-0.5">
                                            UOM: {safeString(item.uom || 'NOS')}
                                        </div>
                                    </td>
                                    <td className="px-3 py-2">
                                        <div className="flex flex-col items-center gap-0.5">
                                            <div className="flex items-center justify-center gap-1.5">
                                                <button
                                                    onClick={() => handleGroupUpdate(groupKey, itemQty - 1)}
                                                ...
                                                {isEditing ? (
                                                    <input
                                                        ...
                                                        onBlur={() => saveQtyEdit(groupKey)}
                                                        onKeyDown={(e) => handleQtyKeyDown(e, groupKey)}
                                                    ...
                                                    <span 
                                                        onClick={() => startEditQty(groupKey, itemQty)}
                                            ...
                                            <span className="text-[9px] text-gray-500 leading-none">{safeString(item.uom || 'NOS')}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-3 py-2 text-center">
                                        <button
                                            ...
                                            title="Remove all items with this barcode"
                                        >
                                            ✕
                                        </button>
                                    </td>
...
const RetailPos = () => {
    ...
    // Removes exactly one cart line, identified by its unique `id`.
    // Never matches by product id alone, so lines sharing a product
    // (barcoded vs non-barcoded, or different barcodes) are never
    // confused with one another.
    const removeItem = useCallback((lineId) => {
        setCart(prev => {
            if (!prev || !Array.isArray(prev)) return [];
            return prev.filter(i => i && i.id !== lineId);
        });
    }, []);

    // Updates the quantity of exactly one cart line, identified by its
    // unique `id`.
...
const handleAddToCart = useCallback(async (product, quantity, customPrice = null) => {
            ...
            try {
                ...
                for (const item of (bill.items || [])) {
                    try {
                        ...
                        // Check if this item has barcode info
                        let barcodeDetails = null;
                        if (item.barcodeId) {
                            try {
                                const barcodeRes = await axios.get(`${API}/barcodes/${item.barcodeId}`, {
                                    params: { companyId }
                                });
                                barcodeDetails = barcodeRes.data;
                            } catch (barcodeError) {
        ...
        try {
            const payload = {
                ...
                items: cart.map(i => ({
                    ...
                    uom: i.uom || 'NOS',
                    barcode: i.barcodeDetails?.barcode || null,
                    barcodeId: i.barcodeDetails?._id || null,
                    expiryDate: i.barcodeDetails?.expiryDate || null
                })),
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [productsRes, barcodesRes] = await Promise.all([
                axios.get(`${API}/products?companyId=${companyId}`, { timeout: 8000 }),
            ...
            const activeProducts = productsRes.data.filter(product => product.isActive === true);
            setProducts(activeProducts);
            setBarcodes(barcodesRes.data.data || []);
            
            console.log('Products loaded:', activeProducts.length);
            console.log('Barcodes loaded:', barcodesRes.data.data?.length);
        } catch (error) {
    ...
    // Updates the price for every underlying cart line belonging to the
    // same product + barcode group as the edited (grouped) display row.
    // This intentionally applies to the whole group (unlike remove/qty,
    // which act on a single line), since price editing is meant to
    // reprice all merged lines of that exact product/barcode together.
    const handleUpdatePrice = useCallback(() => {
        ...
        const editBarcodeId = normalizeBarcodeId(selectedEditItem.barcodeDetails);

        setCart(prevCart => prevCart.map(item => {
            const itemBarcodeId = normalizeBarcodeId(item.barcodeDetails);
            if (item.product === selectedEditItem.product && itemBarcodeId === editBarcodeId) {
    ...
    useEffect(() => {
        ...
        fetchData();
        fetchCustomers();
    }, [companyId, fetchData, fetchCustomers, navigate]);
                ...
                >
                    ...
                    >
                        <div className="p-5 border-b">
                            ...
                            {selectedEditItem.barcodeDetails && (
                                <p className="text-xs text-indigo-500 font-mono mt-1">
                                    📱 {selectedEditItem.barcodeDetails.barcode}
                                    {selectedEditItem.barcodeDetails.expiryDate && (
                                        <span className="text-gray-400 ml-2">
                                            Exp: {new Date(selectedEditItem.barcodeDetails.expiryDate).toLocaleDateString()}
                                        </span>
                                    )}
                                </p>
                            )}
